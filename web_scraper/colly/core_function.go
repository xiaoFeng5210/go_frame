...
var (
	// link = "https://www.bilibili.com/video/BV1EEWYeVEhR"
	link = "https://www.cnblogs.com/zhangchaoyang/p/17731952.html"
)
...
func collyGet1() {
	...
	if err := c.Head(link); err == nil {
		if err := c.Visit(link); err != nil { //Visit执行的是GET请求。colly同样支持Post(URL string, requestData map[string]string)和PostRaw(URL string, requestData []byte)
			fmt.Println("visit fail", err)
		}
	} else {
		fmt.Println("head fail", err)
	}
}
